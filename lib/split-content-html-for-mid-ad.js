 ...
 *
 * 1) `options.midAdBeforeText`가 있으면, 해당 문자열을 포함하는 `<h2>…</h2>` **바로 앞**에서 자른다.
 * 2) 아니면 `</p>` 개수가 2개 이상이면 절반 지점의 `</p>` 뒤에서 자른다.
 * 3) 그렇지 않고 `</h2>`가 있으면 첫 번째 `</h2>` 뒤에서 자른다(짧은 글·단락 없는 구조 대비).
 *
 * @param {string} html
 * @param {{ midAdBeforeText?: string }} [options]
