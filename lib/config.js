 ...
 * - `app/layout.js` `<head>`에 `<meta name="naver-site-verification" …>` 로 직접 출력
 */
export const NAVER_SITE_VERIFICATION =
  "4cac433695bdf13c6cae8a78e8ab2ff953678afb";

/**
 * Google AdSense 퍼블리셔 ID — `metadata.other`, 스크립트, `public/ads.txt` 공통.
 * 애드센스 계정을 바꿀 때 이 값과 public/ads.txt 한 줄을 함께 수정하세요.
 */
export const ADSENSE_PUBLISHER_ID = "ca-pub-6446845535425039";

/**
 * @returns {string | null} `ca-pub-…` 형식이면 ID, 아니면 null
 */
export function getAdsensePublisherId() {
  const raw = ADSENSE_PUBLISHER_ID.trim();
  return /^ca-pub-\d{16}$/.test(raw) ? raw : null;
}

/**
 * ads.txt 한 줄용 — `ca-` 접두어 제거한 `pub-…`
 * @returns {string | null}
 */
export function getAdsensePubIdForAdsTxt() {
  const id = getAdsensePublisherId();
  if (!id) return null;
  return id.startsWith("ca-") ? id.slice(3) : id;
}
...
export const AUTHOR_NAME = "아이다호 편집자";
 ...
 * `private: true`이면 목록·네비·사이트맵·검색에서 제외하고, 글 URL은 404로 둡니다.
 * @type {{ slug: string; label: string; private?: boolean }[]}
 */
export const SITE_CATEGORY_DEFS = [
  /** 애드센스 심사·브랜드 집중: 목록·URL·사이트맵에서 제외 */
